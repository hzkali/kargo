	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="kargo_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.kargo_post_editor' );
					if ( link.length > 0 ) {
						kargo_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.kargo_post_editor', function(e) {
					kargo_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('kargo_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function kargo_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
