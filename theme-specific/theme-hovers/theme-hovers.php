...
if ( ! function_exists( 'kargo_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'kargo_hovers_theme_setup3', 3 );
	function kargo_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		kargo_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'kargo' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'kargo' ) ),
					'std'     => 'slide_left',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'kargo' ),
						'slide_left'   => esc_html__( 'Slide from Left', 'kargo' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'kargo' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'kargo' ) ),
					'std'      => 'dots',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'kargo' ),
					),
					'options'  => kargo_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'kargo_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'kargo_hovers_theme_setup9', 9 );
	function kargo_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'kargo_hovers_frontend_scripts', 1010 );
		add_filter( 'kargo_filter_localize_script', 'kargo_hovers_localize_script' );
		add_filter( 'kargo_filter_merge_scripts', 'kargo_hovers_merge_scripts' );
		add_filter( 'kargo_filter_merge_styles', 'kargo_hovers_merge_styles' );
		add_filter( 'kargo_filter_merge_styles_responsive', 'kargo_hovers_merge_styles_responsive' );
		add_filter( 'kargo_filter_get_css', 'kargo_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'kargo_hovers_frontend_scripts' ) ) {
	function kargo_hovers_frontend_scripts() {
		if ( kargo_is_on( kargo_get_theme_option( 'debug_mode' ) ) ) {
			$kargo_url = kargo_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $kargo_url ) {
				wp_enqueue_script( 'kargo-hovers', $kargo_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'kargo_hovers_merge_styles' ) ) {
	function kargo_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'kargo_hovers_merge_styles_responsive' ) ) {
	function kargo_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'kargo_hovers_localize_script' ) ) {
	function kargo_hovers_localize_script( $arr ) {
		$arr['button_hover'] = kargo_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'kargo_hovers_merge_scripts' ) ) {
	function kargo_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'kargo_hovers_add_icons' ) ) {
	function kargo_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php kargo_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'kargo' );
					} else {
						esc_html_e( 'Details', 'kargo' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php kargo_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php kargo_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php kargo_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php kargo_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'kargo_hovers_get_css' ) ) {
	function kargo_hovers_get_css( $css, $args ) {
...
/* Slide */
.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['alter_link2']} 50%, {$colors['text_link2']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link2']} !important; }
...
/*
.sc_button_hover_style_link2.sc_button_hover_slide_left {	background: linear-gradient(135deg,	{$colors['text_hover2']} 50%, {$colors['text_link2']} 50%) no-repeat scroll right bottom / 230% 100% {$colors['text_link2']} !important; }*/

.trx_addons_scroll_to_top.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_link2']} 50%, {$colors['bd_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['bd_color']} !important; }
.sc_button_hover_style_link2.sc_button_hover_slide_left {	background: linear-gradient(135deg,	{$colors['extra_link3']}, {$colors['extra_hover2']} 50%, {$colors['extra_hover2']}, {$colors['extra_link3']} 75%) no-repeat scroll right bottom / 230% 100% {$colors['extra_hover2']} !important; }
.sc_action_item_link.sc_button_hover_slide_left {	background: linear-gradient(135deg,	{$colors['extra_link3']}, {$colors['extra_hover2']} 50%, {$colors['extra_hover2']}, {$colors['extra_link3']} 75%) no-repeat scroll right bottom / 230% 100% {$colors['extra_hover2']} !important; }
.team_member_page .team_member_socials .social_item .sc_button_hover_slide_left {	background: linear-gradient(135deg,	{$colors['extra_link3']}, {$colors['extra_hover2']} 50%, {$colors['extra_hover2']}, {$colors['extra_link3']} 75%) no-repeat scroll right bottom / 230% 100% {$colors['extra_hover2']} !important; }
.sc_services_timeline .sc_services_item_marker {	background: linear-gradient(135deg,	{$colors['extra_link3']}, {$colors['extra_hover2']} 50%, {$colors['extra_hover2']}, {$colors['extra_link3']} 75%) no-repeat scroll right bottom / 230% 100% {$colors['extra_hover2']} !important; }
.trx_addons_popup .sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['extra_hover3']} 50%, {$colors['text_link2']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link2']} !important; }
...
.sc_button_hover_style_link3.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['input_bg_hover']} 50%, {$colors['alter_link2']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link2']} !important; }
...
.sc_button_hover_style_dark.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link2']} 50%, {$colors['alter_bg_hover']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_bg_hover']} !important; }
.post_item .sc_button_hover_style_dark.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link2']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['bg_color']} !important; }
.widget_slider .sc_button_hover_style_dark.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link2']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['bg_color']} !important; }
...
.sc_button_hover_style_hover.sc_button_hover_slide_left {		background: linear-gradient(	135deg,	{$colors['extra_link3']}, {$colors['extra_hover2']} 50%, {$colors['extra_hover2']}, {$colors['extra_link3']} 75%) no-repeat scroll right bottom / 230% 100% {$colors['extra_hover2']} !important; }
...
.sc_button_hover_style_alterbd.sc_button_hover_slide_left {		background: linear-gradient(135deg,	{$colors['extra_hover2']}, {$colors['extra_link3']} 50%, {$colors['alter_bd_color']} 50%) no-repeat scroll right bottom / 275% 100% {$colors['alter_bd_color']} !important; }
...
.post_item .sc_button_hover_style_dark.sc_button_hover_slide_left:hover,
.widget_slider .sc_button_hover_style_dark.sc_button_hover_slide_left:hover,
.team_member_page .team_member_socials .social_item .sc_button_hover_slide_left:hover {		background-position: left bottom !important; color: {$colors['inverse_link']} !important; }
...
.post_layout_excerpt .more-link.sc_button_hover_slide_left:hover,
.post_layout_chess .more-link.sc_button_hover_slide_left:hover {
	color: {$colors['text_link2']} !important;
}
.portfolio_filters .sc_button_hover_style_alterbd.sc_button_hover_slide_left {
	color: {$colors['alter_bg_hover']}!important;
	background: {$colors['bd_color']}!important;
}
.portfolio_filters .ui-tabs-active .sc_button_hover_style_alterbd.sc_button_hover_slide_left {
	color: {$colors['inverse_text']}!important;
	background: {$colors['text_link2']}!important;
}
.portfolio_filters .sc_button_hover_style_alterbd.sc_button_hover_slide_left:hover {
	color: {$colors['inverse_text']}!important;
	background: {$colors['text_link2']}!important;
}
.sc_button_alter.sc_button_hover_slide_left {
	color: {$colors['text_link']}!important;
	background: transparent!important;
}
.sc_button_alter.sc_button_hover_slide_left:hover {
...
.scheme_dark .sc_button_alter.sc_button_hover_slide_left:hover:before {
	background: {$colors['text_link2']};
}
.scheme_dark .sc_button_alter.sc_button_hover_slide_left:hover:after {
	color: {$colors['inverse_text']};
}

.color_style_link2.sc_button_hover_slide_left:hover,
.color_style_link2.sc_button_hover_slide_left.active,
.ui-state-active .color_style_link2.sc_button_hover_slide_left,
.vc_active .color_style_link2.sc_button_hover_slide_left,
.vc_tta-accordion .vc_tta-panel-title:hover .color_style_link2.sc_button_hover_slide_left,
li.active .color_style_link2.sc_button_hover_slide_left {		background-position: left bottom !important; color: {$colors['inverse_link']} !important; }
...
.post_featured.hover_slide .post_info .post_meta_item.post_categories {
	color: {$colors['text_link']};
}
.post_featured.hover_slide .post_info .post_meta_item.post_categories a {
	color: {$colors['text_link']};
}
.post_featured.hover_slide .post_info .post_meta_item.post_categories a:hover {
