...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'kargo_trx_addons_theme_specific_setup1' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'kargo_trx_addons_theme_specific_components' );
	function kargo_trx_addons_theme_specific_components( $enable = false ) {
		return KARGO_THEME_FREE
					? false     // Free version
...
if ( ! function_exists( 'kargo_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'kargo_trx_addons_theme_specific_setup1', 1 );
	function kargo_trx_addons_theme_specific_setup1() {
		if ( kargo_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'kargo_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'kargo_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'kargo_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'kargo_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'kargo_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'kargo_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'kargo_trx_addons_widgets_list' );
		}
	}
}

// CV
if ( ! function_exists( 'kargo_trx_addons_cv_enable' ) ) {
	function kargo_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'kargo_trx_addons_demo_enable' ) ) {
	function kargo_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'kargo_trx_addons_edd_themes_market_enable' ) ) {
	function kargo_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'kargo_trx_addons_api_list' ) ) {
	function kargo_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'kargo_trx_addons_cpt_list' ) ) {
	function kargo_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'kargo_trx_addons_sc_list' ) ) {
	function kargo_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'kargo_trx_addons_widgets_list' ) ) {
	function kargo_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'kargo_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'kargo_trx_addons_menu_cache' );
	function kargo_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'kargo_trx_addons_localize_script' ) ) {
	...
	function kargo_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = kargo_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'kargo_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'kargo_trx_addons_sc_type', 10, 2 );
	function kargo_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = kargo_array_merge( $list, kargo_get_list_blog_styles( false, 'sc' ) );
		}
		...
		// new type sc_button
		if ($sc == 'trx_sc_button') {
			$list['greyscale'] = 'Bordered Greyscale';
		}
		...
		// new type sc_layouts_iconed_text
...
if ( ! function_exists( 'kargo_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'kargo_trx_addons_sc_prepare_atts', 10, 2 );
	function kargo_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = kargo_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = kargo_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'kargo_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'kargo_trx_addons_sc_atts', 10, 2 );
	function kargo_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'kargo_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'kargo_trx_addons_sc_map', 10, 2 );
	function kargo_trx_addons_sc_map( $params, $sc ) {
		...
		if ( in_array(
			...
			$params['params'][] = array(
				'param_name'  => 'scheme',
				'heading'     => esc_html__( 'Color scheme', 'kargo' ),
				'description' => wp_kses_data( __( 'Select color scheme to decorate this block', 'kargo' ) ),
				'group'       => esc_html__( 'Colors', 'kargo' ),
				'admin_label' => true,
				'value'       => array_flip( kargo_get_list_schemes( true ) ),
				'type'        => 'dropdown',
			);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'kargo' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'kargo' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( kargo_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$styles = (array)kargo_storage_get( 'blog_styles' );
			unset( $styles['gallery'] );  // On gallery hover is 'icon' always
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'kargo' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'kargo' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( kargo_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'kargo_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'kargo_trx_addons_sow_map', 10, 2 );
	function kargo_trx_addons_sow_map( $params, $sc ) {
		...
		if ( in_array( $sc, array( 'trx_sc_button' ) ) ) {
			kargo_array_insert_after( $params, 'size', $param );
		} elseif ( in_array(
			$sc, array(
				'trx_sc_action',
				'trx_sc_blogger',
			...
			kargo_array_insert_after( $params, 'title_align', $param );
		}
		return $params;
	}
}
...
if ( ! function_exists( 'kargo_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'kargo_trx_addons_sc_output', 10, 4 );
	function kargo_trx_addons_sc_output( $output, $sc, $atts, $content ) {

		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! kargo_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_action ', 'class="sc_action scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! kargo_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'kargo_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'kargo_trx_addons_sc_item_title_tag' );
	function kargo_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'kargo_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'kargo_trx_addons_sc_item_button_args', 10, 3 );
	function kargo_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'kargo_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'kargo_trx_addons_slider_title', 10, 2 );
	function kargo_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'kargo_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'kargo_trx_addons_sc_googlemap_styles' );
	function kargo_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'kargo' );
		return $list;
	}
}
...
if ( ! function_exists( 'kargo_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'kargo_trx_addons_tiny_mce_style_formats' );
	function kargo_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'kargo' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'kargo' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'kargo_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'kargo_filter_allow_override_header_image', 'kargo_trx_addons_allow_override_header_image' );
	function kargo_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					kargo_is_team_page()
					|| kargo_is_cars_page()
					|| kargo_is_cars_agents_page()
					|| kargo_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'kargo_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'kargo_trx_addons_thumb_size', 10, 2 );
	function kargo_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'kargo_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'kargo_trx_addons_meta_box_fields', 10, 2 );
	function kargo_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'kargo' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'kargo' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
